...
workflow {
    ...
    ch_version_files = ch_mock_versions
        .collectFile() { content ->
    ...
    ch_processed_versions = ch_version_files
        .unique()                                    // Channel logic stays in pipeline
        .map { version_file -> 
            log.info "Processing version file: ${version_file}"
            processVersionsFromFile([version_file.toString()])  // Plugin utility function
        }
        .unique()                                    
        .mix(                                        // Channel orchestration visible
            Channel.of(getWorkflowVersion())         // Plugin utility function
                .map { workflow_version ->
                    log.info "Adding workflow version: ${workflow_version}"
                    // Format as YAML to match legacy workflowVersionToYAML() output
                    """
                    Workflow:
                        ${workflow.manifest.name ?: 'nf-core-utils-validation'}: ${workflow_version}
